...
export const useIndexStore = defineStore(
    ...
    () => {
        ...
        const projects = useCollection<Project>(
            query(collection(db, 'projects'), where('user', '==', user.value?.uid)),
            {
                ssrKey: 'projects',
            },
        );
        ...
        async function updateEntry(updatedEntry: Entry, originalEntry: Entry) {
            await updateDoc(doc(db, 'entries', originalEntry.id), {
                ...updatedEntry,
                project: doc(db, 'projects', updatedEntry.project.id),
                date: $moment(updatedEntry.date).startOf('day').toDate(),
            });
        }
    ...
    {
        persist: {
            paths: ['selectedDay', 'menuOpened', 'filter', 'selectedTabIndex', 'sort', 'weekObjective'],
        },
    },
);
